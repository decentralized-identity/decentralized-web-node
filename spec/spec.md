...
`did:example:123` + `?service=IdentityHub` + `&messages=` + `[{ MESSAGE_1 }, { MESSAGE_N }]`
...
```json
did:example:123?service=IdentityHub&messages=[{...}, { "descriptor": { "method": "CollectionsQuery", "schema": "https://schema.org/SocialMediaPosting" }}]
```
...
3. The *Request Object* ****MUST**** include a `requestId` property, and its value ****MUST**** be an [[spec:rfc4122]] UUID Version 4 string.
...
1. The object ****MUST**** include a `requestId` property, and its value ****MUST**** be the [[spec:rfc4122]] UUID Version 4 string from the `requestId` property of the [*Request Object*](#request-object) it is in response to.
